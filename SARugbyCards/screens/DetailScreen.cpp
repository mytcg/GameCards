...
DetailScreen::DetailScreen(MainScreen *previous, Feed *feed, int screenType, Card *card, String category, String categoryname) : mHttp(this),
	...
	switch (screenType) {
		case PROFILE:
			...
			break;
		case BALANCE:
			label = new Label(0,0, scrWidth-PADDING*2, DEFAULT_DETAIL_HEADER_HEIGHT, NULL, "Go to sarugbycards.com to find out how to get more credits.", 0, Util::getDefaultSelected());
			label->setAutoSizeY();
			label->setMultiLine(true);
			label->setDrawBackground(false);
			kinListBox->add(label);
