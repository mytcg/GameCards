...
GCMenu::GCMenu(item items[], int numItems, int x, int y, int width, int height,
	...
	int imageHeight = height - ICON_HEIGHT - DOT_HEIGHT - PADDING;
	...
	ListBox *iconAndDotList = new ListBox(0, 0, width, ICON_HEIGHT + DOT_HEIGHT + PADDING, listBox,
			ListBox::LBO_VERTICAL);
	iconAndDotList->setSkin(Util::getIconListBack());

	iconList = new ListBox(0, 0, width, ICON_HEIGHT + PADDING, iconAndDotList,
		ListBox::LBO_HORIZONTAL);
	iconList->setDrawBackground(false);
	iconList->setPaddingTop(PADDING);
...
void GCMenu::selectNext() {
	if (!emp) {
		((Image*)iconLists[selectedList]->getChildren()[iconLists[selectedList]->getSelectedIndex()])->setResource(
				items[iconLists[selectedList]->getSelectedIndex() + (iconsPerList * selectedList)].icon);
		...
		((Image*)iconLists[selectedList]->getChildren()[iconLists[selectedList]->getSelectedIndex()])->setResource(
				items[iconLists[selectedList]->getSelectedIndex() + (iconsPerList * selectedList)].iconSel);
