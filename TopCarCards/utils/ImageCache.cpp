...
void ImageCache::connRecvFinished(MAUtil::Connection* conn, int result) {
	if (result >= 0) {
		...
		if (mNextRequest->getImage() != NULL) {
			if (mNextRequest->getType() == 0) {
				//normal loading
			} else if ((((mNextRequest->getType() == 3)||(mNextRequest->getType() == 4))&&portrait)||(((mNextRequest->getType() == 1)||(mNextRequest->getType() == 2))&&!portrait)||((mNextRequest->getType() == 7)&&!portrait)) {
				if (val <= 5.5) {
					maDestroyObject(mNextRequest->getImage()->getResource());
				...
				mNextRequest->getImage()->requestRepaint();
			} else if ((((mNextRequest->getType() == 1)||(mNextRequest->getType() == 2))&&portrait)||(((mNextRequest->getType() == 3)||(mNextRequest->getType() == 4))&&!portrait)||((mNextRequest->getType() == 7)&&portrait)) {
				if (val <= 5.5) {
					maDestroyObject(mNextRequest->getImage()->getResource());
