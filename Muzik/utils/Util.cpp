...
WidgetSkin* Util::getSkinHeader() {
	...
	if (gSkinHead == NULL) {
		...
		gSkinHead = new WidgetSkin(RES_CROSSLOGO_BACKGROUND, RES_CROSSLOGO_BACKGROUND,
				HEAD_X_LEFT, HEAD_X_RIGHT, HEAD_Y_TOP, HEAD_Y_BOTTOM, false, false);
	}
	return gSkinHead;
}
...
Widget* Util::createSoftKeyBar(int height, const char *left, const char *right, const char *centre) {
	...
	layout->setPaddingBottom(10);
	layout->setPaddingLeft(10);
	layout->setPaddingRight(10);
	layout->setPaddingTop(10);

	Button *label = new Button(0,0, (scrWidth - 20)/3, layout->getHeight() - 20, NULL);
	...
	label = new Button(0,0, (scrWidth - 20)/3, layout->getHeight() - 20, NULL);
