...
card(card), screenType(screenType), detail(detail) {
	...
	switch (screenType) {
		case ST_CARD_NOTE:
			mainLayout = Util::createMainLayout("Stoor", "Terug", "", true);
			break;
		case ST_SMS:
			mainLayout = Util::createMainLayout("Send", "Terug", "", true);
		break;
	}
	...
	switch (screenType) {
		case ST_CARD_NOTE:
			...
			label->setAutoSizeX();
			label->setMultiLine();
			label = new Label(0,0, scrWidth-PADDING*2, DEFAULT_SMALL_LABEL_HEIGHT, NULL, "Nota", 0, Util::getDefaultFont());
			label->setDrawBackground(false);
	...
	editBoxNote = new NativeEditBox(0, 0, label->getWidth()-PADDING*2, label->getHeight()-PADDING*2,140, MA_TB_TYPE_ANY, label, "",L"Nota");
