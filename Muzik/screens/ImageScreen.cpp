...
ImageScreen::ImageScreen(MainScreen *previous, MAHandle img, Feed *feed, bool flip, Card *card, int screenType, bool hasConnection,
	...
	if (card != NULL) {
		if (screenType == ST_NEW_CARD) {
			...
			mainLayout =  Util::createImageLayout("Add", "Terug" , "Draai");
		}
		else if(screenType == ST_DECK_REMOVE){
			mainLayout =  Util::createImageLayout("Remove", "Terug" , "Draai");
		}
		else if(screenType == ST_DECK_OPTIONS){
			mainLayout =  Util::createImageLayout("Keuses", "Terug" , "Draai");
		}
		else if(screenType == ST_DECK_ADDON){
			mainLayout =  Util::createImageLayout("Unequip", "Terug" , "Draai");
		}
		else if(screenType == ST_DECK_ADDON_EQUIP){
			mainLayout =  Util::createImageLayout("Equip", "Terug" , "Draai");
		}
		else {
			mainLayout =  Util::createImageLayout((hasConnection&&canAuction)?"Keuses":"", "Terug" , "Draai");
		}
...
void ImageScreen::keyPressEvent(int keyCode) {
	...
	switch (keyCode) {
		case MAK_LEFT:
			if(currentSelectedKey!=NULL){
				...
				if (screenType == ST_NEW_CARD) {
					...
					Util::updateSoftKeyLayout((hasConnection&&canAuction)?"Keuses":"", "Terug", "Draai", mainLayout);
				}
				imge->flip();
				//imge->refreshWidget();
				//imge->statAdded = false;
		...
		case MAK_UP:
			...
			}else if(card->getStats().size()>0 && portrait){
				if (imge->getResource() != RES_TEMP) {
					selectStat(-1);
					if (currentSelectedStat == -1) {
						imge->refreshWidget();
