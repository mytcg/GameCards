...
DetailScreen::DetailScreen(MainScreen *previous, Feed *feed, int screenType, Card *card, String category, String categoryname) : mHttp(this),
	...
	switch (screenType) {
		case PROFILE:
			label = new Label(0,0, scrWidth-PADDING*2, DEFAULT_DETAIL_HEADER_HEIGHT, NULL, "Verdien krediete deur profiel inligting te verskaf.", 0, Util::getDefaultSelected());
			label->setAutoSizeY();
	...
	if (screenType == PROFILE) {
		...
		if(res < 0) {
			...
			label = (Label *) mainLayout->getChildren()[0]->getChildren()[1];
			label->setCaption("Opdatering van kennisgewings...");
			label->setDrawBackground(false);
