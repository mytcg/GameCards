...
void AuctionCreateScreen::drawDataInputScreen() {
	if (mainLayout == NULL) {
		mainLayout = Util::createMainLayout("Auction", "Terug", "", true);
		...
		currentSelectedKey = NULL;
		currentKeyPosition = -1;
		Util::updateSoftKeyLayout("Auction", "Terug", "", mainLayout);
	}
	...
	label = new Label(0,0, scrWidth-PADDING*2, DEFAULT_SMALL_LABEL_HEIGHT, NULL, "Aanvangsbod", 0, Util::getDefaultFont());
	label->setDrawBackground(false);
	kinListBox->add(label);

	label = Util::createEditLabel("");
	editBoxOpening = new NativeEditBox(0, 0, label->getWidth()-PADDING*2, label->getHeight()-PADDING*2, 64, MA_TB_TYPE_NUMERIC, label, "", L"Aanvangsbod");
	...
	label = new Label(0,0, scrWidth-PADDING*2, DEFAULT_SMALL_LABEL_HEIGHT, NULL, "Duur van veiling (dae)", 0, Util::getDefaultFont());
	label->setDrawBackground(false);
	kinListBox->add(label);

	label = Util::createEditLabel("");
	editBoxDays = new NativeEditBox(0, 0, label->getWidth()-PADDING*2, label->getHeight()-PADDING*2, 64, MA_TB_TYPE_NUMERIC, label, "", L"Duur van veiling (dae)");
...
void AuctionCreateScreen::drawConfirmScreen() {
	...
	String result = "Wil jy " + card->getText() + " opveil?";
