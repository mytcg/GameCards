...
ForgotPasswordScreen::ForgotPasswordScreen(Feed *feed, MainScreen *previous) : mHttp(this) {
	...
	mainLayout = Util::createMainLayout("Herstel", "Terug", "", true);

	kinListBox = (KineticListBox*) mainLayout->getChildren()[0]->getChildren()[2];
	notice = (Label*) mainLayout->getChildren()[0]->getChildren()[1];
	notice->setMultiLine(true);

	label = new Label(0,0, scrWidth-PADDING*2, DEFAULT_SMALL_LABEL_HEIGHT, NULL, "Om jou wagwoord te wysig, sleutel e-pos adres in", 0, Util::getDefaultFont());
	...
	editBoxEmail = new NativeEditBox(0, 0, label->getWidth()-PADDING*2, label->getHeight()-PADDING*2, 64, MA_TB_TYPE_ANY, label, "",L"E-pos adres");
