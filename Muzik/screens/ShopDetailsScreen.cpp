...
ShopDetailsScreen::ShopDetailsScreen(MainScreen *previous, Feed *feed, int screenType, bool free, Product *product, Auction *auction, bool first) : mHttp(this), screenType(screenType), product(product), auction(auction), first(first), free(free) {
	...
	if (screenType == ST_AUCTION)
	{
		if (!strcmp(auction->getBuyNowPrice().c_str(), "")) {
			mainLayout = Util::createMainLayout("", "Terug", "Bid", true);
		} else {
			if((!strcmp(auction->getBuyNowPrice().c_str(), ""))||(!strcmp(auction->getBuyNowPrice().c_str(), "0"))) {
				mainLayout = Util::createMainLayout("", "Terug", "Bid", true);
				buynow = false;
			} else {
				mainLayout = Util::createMainLayout("Buy", "Terug", "Bid", true);
				buynow = true;
			}
		}
	}
		...
		mainLayout = Util::createMainLayout("Bevestig", "Terug", "Cards", true);
	else
		mainLayout = Util::createMainLayout("Koop", "Terug", "Cards", true);
	...
	{
		if (!expired) {
			label = new Label(0,0, scrWidth-PADDING*2, scrHeight - DEFAULT_SMALL_LABEL_HEIGHT, NULL, "Bie", 0, Util::getDefaultFont());
			label->setDrawBackground(false);
			label->setMultiLine();
			label->setAutoSizeY();
			label->setDrawBackground(false);
...
void ShopDetailsScreen::drawBuyNow()
{
	...
	currentKeyPosition = -1;
	if (mainLayout == NULL) {
		mainLayout = Util::createMainLayout("Bevestig", "Terug", true);
		kinListBox = (KineticListBox*) mainLayout->getChildren()[0]->getChildren()[2];
		notice = (Label*) mainLayout->getChildren()[0]->getChildren()[1];
	}
	else {
